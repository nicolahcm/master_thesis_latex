...
In this chapter we will run some numerical tests to get a hold on the hybrid Euler scheme. For the code refer to the Appendix \ref{appendix: hybridEulerScheme}.
\\

We will use with the following parameters (which are also found in real applications, see \cite{Main}):
...
\noindent
The growth of the absolute values of the coefficients $\left\{ |a_k|\right\}_{k\in\mathbb{N}}$ is exponential, confirmed by the plot of $f(k) = \log(|a_k|)$ in Figure \ref{barbaz}.
...
\\
\\
In Figure \ref{fig: psi n=16}, \ref{fig: psi n=64} and \ref{fig: psi n=256} we display some plots of $\bar{\psi}^n$ for different values of $n$ ($n=16,64,256$, respectively).

\begin{figure}[H]
  \includegraphics[width=\textwidth, height= 55mm]{barpsin/n = 16.png}
  \caption{(a) curve of $\bar{\psi}^{16}(t)$.} \label{fig: psi n=16}
\end{figure}

\begin{figure}[H]
  \includegraphics[width=\textwidth, height= 55mm]{barpsin/n = 64.png}
  \caption{(b) curve of $\bar{\psi}^{64}(t)$.} \label{fig: psi n=64}
\end{figure}

\begin{figure}[H]
  \includegraphics[width=\textwidth, height= 55mm]{barpsin/n = 256.png}
  \caption{(c) curve of $\bar{\psi}^{256}(t)$.} \label{fig: psi n=256}
\end{figure}
...
From the plot in Figure \ref{c1plot1} it is more evident that the sequence $\left\{\bar{c}_1^n \right\}_{n\in\mathbb{N}}$ converges.
...
In this particular case, after applying a linear regression, we obtained that a coefficient of $-0.68382593$ is fitting the curve. 
...
\\
However, in all of this, it is noted in \cite{Main} (in Section $4.5$) that the regression coefficient changes as we consider wider ranges of values for $n$.
\\
...
From Table \ref{c1table} we observe that the computation time of $\bar{c}_1^n$ increases exponentially with respect to $n$: if $n$ doubles, the computational time of evaluation of $\bar{c}_1^n$ quadruples.
\\
...
Applying a linear regression to the curve in Figure \ref{comptimes} yields a coefficient of $1.99993845$ that fits the curve. We could have already expected it by looking at the results in Table \ref{c1table} (for $n=2^{15}$, computing $\bar{c_1}^n$ requires about fifteen minutes, while for  $n=2^{16}$ it takes an hour and for $n=2^{17}$ it takes four hours of computation).
