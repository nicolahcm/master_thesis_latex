...
These observations suggest that the RR estimate of second order (eq. \eqref{eq: moreAccurateEstimate}) and the estimate in equation \eqref{eq: estimateRRwithbeta} do not hold true. However, all this does not imply that the double step RR extrapolation (described in Subsection \ref{subsection: multiStepRR} and equation \eqref{eq: multiStepRR}) is useless: we remind that $\left\{ \bar{\psi}^n_{_{RR,3}} \right\}_{n \in \mathbb{N}}$ does converge to $\psi$ (as $n\to +\infty$), but the convergence rate \eqref{eq: rr3 convergence rate} might not hold true since the second order RR estimate \eqref{eq: moreAccurateEstimate}, from which it follows, seems not to hold. 
\\
...
) in terms of computation times\footnote{In seconds. We remind that we use a computer with the following properties: 12 CPUs i7-8750H  @ 2.20GHz. Even though the absolute times might change in different machines, we are more interested in ratios among different computation times.} and convergence rate (as $n\to +\infty$). 
\\
\\
We can observe from Table \ref{tavolarichardsonrombergtest1} and Figure \ref{comparisonComExtr} the values of $\bar{\psi}^n_{_{RR,2}}(T)$ and $\bar{\psi}^n_{_{RR,3}}(T)$ (computed through equations \eqref{eq: multiStepRR} and \eqref{eq: RR.1}, respectively) and their relative computation times in seconds (the columns "comp. time RR2" and "comp. time RR3" of Table \ref{tavolarichardsonrombergtest1}). 
\\
\\
We can notice that $\bar{\psi}^n_{_{RR,3}}(T)$ takes slightly more time than $\bar{\psi}^n_{_{RR,2}}(T)$ to be computed. This could have been expected. Indeed, reminding equations \eqref{eq: multiStepRR} and \eqref{eq: RR.1}, ....
, it is clear that... This will surely be more visible with higher values of $n$.
\textcolor{red}{ as we could have expected: we need to compute a more term: Riscrivi le formule.}
\\
\\
However, $\bar{\psi}^n_{_{RR,3}}(T)\right\}$ seems to converge faster than $\bar{\psi}^n_{_{RR,2}}(T) \right\}$ (for $n \to +\infty$), as we can observe from Figure \ref{comparisonComExtr}. in function of $n$.
\\
\\
So deciding whether to use one or the other is up to convenience: if we need fast convergence and we have no problems of powerful computers, then we can go for the RR3. If we need a quick computation and not a very precise estimate, we can go for RR2. 
%\\ 
%The numerical validity is given by the following argument:
%the sequence of estimates $\left\{ \bar{\psi}^n_{_{RR,3}} \right\}_{n \in %\mathbb{N}}$ of $\psi$ seems to converge (as $n\to +\infty$) to the same %limit as that of the sequence $\left\{ \bar{\psi}^n_{_{RR,2}} \right\}_{n %\in \mathbb{N}}$ given by the single step RR extrapolation method
%(described in Section \ref{section: extrapolatedhyb} and equation
 %\eqref{eq: RR.1}), namely
%\begin{equation}
...
%This result (equation \eqref{eq: rr2samelimitrr3}) can observed from Table \ref{tavolarichardsonrombergtest1} and Figure \ref{comparisonComExtr} from which we can observe the values of $\bar{\psi}^n_{_{RR,2}}(T)$ and $\bar{\psi}^n_{_{RR,3}}(T)$ computed through equations \eqref{eq: multiStepRR} and \eqref{eq: RR.1}. Moreover we have the computation times in the third and fourth column, where we can observe that $\bar{\psi}^n_{_{RR,3}}(T)$ takes slightly more time than $\bar{\psi}^n_{_{RR,2}}(T)$ to be computed. However, $\bar{\psi}^n_{_{RR,3}}(T)\right\}$ seems to converge faster than $\bar{\psi}^n_{_{RR,2}}(T) \right\}$ (for $n \to +\infty$), as we can observe from Figure \ref{comparisonComExtr}.


\textcolor{red}{so, even if those RR estimate 2nd order does not hold true, the RR3 can be proven useful since it converges faster for $n$ but it takes slightly more time to be comouted. It is to be evaluated from time to time....}
...
%\textcolor{red}{fare test numerici per vedere se rr2 e rr3 di T valgono effettivamente come il metodo ibrido! Non servono ma mettili lo stesso i cosi numerici. Prova teoreticamente che RR2 e RR3 convergono effettivamente a psi di T...}
%\\
%............
%\\
